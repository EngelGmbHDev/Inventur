...
export function createRepo(path, schemaPath) {
  ...
  return {
    ...
    importRun(runId, rows) {
      const insL = db.prepare('INSERT INTO lines(run_id,n,lagerplatz,itemcode,buchbestand,whscode,price) VALUES(?,?,?,?,?,?,?)');
      tx(() => {
        run('DELETE FROM lines WHERE run_id=?', runId);
        run('DELETE FROM tasks WHERE run_id=?', runId);
        for (const r of rows) insL.run(runId, r.n, r.lagerplatz, r.itemcode, r.buchbestand, r.whscode, r.price);
    ...
    exportRows: (runId) =>
      all(`SELECT l.n, l.lagerplatz, l.itemcode, l.itemcode_soll, l.added, l.menge, l.buchbestand, l.whscode, l.price, l.counted_at, t.worker
